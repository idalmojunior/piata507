...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function loadWeather() {
    ...
    try {
      ...
      el.innerHTML = `<div class="weather-loading">🌊 Dados de clima indisponíveis no momento</div>`;
    }
  }

  // ── Tide navigator (7 days, prev/next) ────
  function generateTideData() {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      const events = base.map(t => {
        const [hh, mm] = t.time.split(':').map(Number);
        const total = hh * 60 + mm + shift;
        const wrapped = ((total % 1440) + 1440) % 1440;
        return { ...t, time: `${String(Math.floor(wrapped/60)).padStart(2,'0')}:${String(wrapped%60).padStart(2,'0')}` };
      });
  ...
  function renderTide() {
    const day = tideData[tideIndex];
    const dateEl = document.getElementById('tideDate');
    const itemsEl = document.getElementById('tideItems');
    const dotsEl = document.getElementById('tideDots');
    if (!dateEl || !itemsEl) return;

    const labels = ['Hoje', 'Amanhã'];
    const dateLabel = tideIndex < 2
      ? labels[tideIndex]
      : day.date.toLocaleDateString('pt-BR', { weekday: 'long', day: '2-digit', month: '2-digit' });

    dateEl.textContent = dateLabel.charAt(0).toUpperCase() + dateLabel.slice(1);

    itemsEl.innerHTML = day.events.map(e => `
      <div class="tide-item">
        <div class="tide-item-time ${e.type === 'low' ? 'tide-low-c' : 'tide-high-c'}">${e.type === 'low' ? '🔵' : '🟠'} ${e.time}</div>
        <div class="tide-item-label">${e.type === 'low' ? 'Maré Baixa' : 'Maré Alta'} · ${e.h}m</div>
      </div>
    `).join('');

    if (dotsEl) {
      dotsEl.innerHTML = tideData.map((_, i) =>
        `<span class="tide-dot ${i === tideIndex ? 'active' : ''}" data-i="${i}"></span>`
      ).join('');
      dotsEl.querySelectorAll('.tide-dot').forEach(dot => {
        dot.addEventListener('click', () => { tideIndex = Number(dot.dataset.i); renderTide(); });
      });
    }

    document.getElementById('tidePrev').disabled = tideIndex === 0;
    document.getElementById('tideNext').disabled = tideIndex === tideData.length - 1;
  }

  document.getElementById('tidePrev')?.addEventListener('click', () => {
    if (tideIndex > 0) { tideIndex--; renderTide(); }
  });
  document.getElementById('tideNext')?.addEventListener('click', () => {
    if (tideIndex < tideData.length - 1) { tideIndex++; renderTide(); }
  });
